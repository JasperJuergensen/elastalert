...
from elastalert.alerter.jira_alerter import JiraFormattedMatchString
...
def test_email():
    ...
    with mock.patch("elastalert.alerter.email_alerter.SMTP") as mock_smtp:
        ...
        expected = [
            ...
            mock.call().sendmail(
                mock.ANY, ["testing@test.test", "test@test.test"], mock.ANY
            ),
            mock.call().quit(),
        ]
...
def test_email_with_auth():
    ...
    with mock.patch("elastalert.alerter.email_alerter.SMTP") as mock_smtp:
        with mock.patch("elastalert.alerter.alerter.yaml_loader") as mock_open:
...
def test_email_with_cc():
    ...
    with mock.patch("elastalert.alerter.email_alerter.SMTP") as mock_smtp:
        ...
        expected = [
            ...
            mock.call().sendmail(
                mock.ANY,
                ["testing@test.test", "test@test.test", "tester@testing.testing"],
                mock.ANY,
            ),
            mock.call().quit(),
        ]
...
def test_opsgenie_basic():
    ...
    with mock.patch("requests.post") as mock_post:
        ...
        assert mcal[0][1]["json"]["responders"] == [
            {"username": "lytics", "type": "user"}
        ]
        assert mcal[0][1]["json"]["source"] == "ElastAlert"
...
def test_opsgenie_details_with_constant_value():
    ...
    mock_post_request.assert_called_once_with(
        ...
        headers={"Content-Type": "application/json", "Authorization": "GenieKey ogkey"},
        json=mock.ANY,
        proxies=None,
    )
...
def test_opsgenie_details_with_field():
    ...
    match = {"message": "Bar", "@timestamp": "2014-10-31T00:00:00"}
    alert = OpsGenieAlerter(rule)
...
def test_opsgenie_details_with_nested_field():
    ...
    match = {"nested": {"field": "Bar"}, "@timestamp": "2014-10-31T00:00:00"}
    alert = OpsGenieAlerter(rule)
...
def test_opsgenie_details_with_non_string_field():
    rule = {
        ...
        "opsgenie_account": "genies",
        "opsgenie_key": "ogkey",
        "opsgenie_details": {"Age": {"field": "age"}, "Message": {"field": "message"}},
    }
    match = {"age": 10, "message": {"format": "The cow goes %s!", "arg0": "moo"}}
    alert = OpsGenieAlerter(rule)
...
def test_jira():
    ...
    with mock.patch("elastalert.alerter.jira_alerter.JIRA") as mock_jira, mock.patch(
        "elastalert.alerter.alerter.yaml_loader"
    ) as mock_open:
    ...
    with mock.patch("elastalert.alerter.jira_alerter.JIRA") as mock_jira, mock.patch(
        ...
        with mock.patch(
            "elastalert.alerter.jira_alerter.JIRA"
        ) as mock_jira, mock.patch(
            "elastalert.alerter.alerter.yaml_loader"
        ) as mock_open:
            mock_open.return_value = {"user": "jirauser", "password": "jirapassword"}
            mock_jira.return_value = mock.Mock()
            ...
            alert = JiraAlerter(rule)
            alert.alert(
                [{"gmail.the_user": "jdoe", "@timestamp": "2014-10-31T00:00:00"}]
            )
...
def test_jira_arbitrary_field_support():
    ...
    rule = {
        ...
        "jira_arbitrary_string_array_field": [
            "arbitrary_string_value1",
            "arbitrary_string_value2",
        ],
        ...
        "jira_arbitrary_complex_array_field": [
            "arbitrary_complex_value1",
            "arbitrary_complex_value2",
        ],
    ...
    with mock.patch("elastalert.alerter.jira_alerter.JIRA") as mock_jira, mock.patch(
        ...
        with pytest.raises(Exception) as exception:
            alert = JiraAlerter(rule)
            alert.alert(
                [{"test_term": "test_value", "@timestamp": "2014-10-31T00:00:00"}]
            )
        assert (
            "Could not find a definition for the jira field 'nonexistent field'"
            in str(exception)
        )
    ...
    with mock.patch("elastalert.alerter.jira_alerter.JIRA") as mock_jira, mock.patch(
        ...
        assert (
            "Exception encountered when trying to add 'invalid_watcher' as a watcher. Does the user exist?"
            in str(exception)
        )
...
def test_kibana(ea):
    ...
    assert time_range["to"] == ts_add(
        match["@timestamp"], datetime.timedelta(minutes=10)
    )
...
def test_command():
    ...
    with mock.patch(
        "elastalert.alerter.command_alerter.subprocess.Popen"
    ) as mock_popen:
        alert.alert([match])
    assert mock_popen.called_with(
        ["/bin/test", "--arg", "foobarbaz"], stdin=subprocess.PIPE, shell=False
    )
    ...
    rule = {
        "command": "/bin/test/ --arg {match[somefield]}",
    ...
    rule = {
        "command": "/bin/test/ --arg {match[nested][field]}",
        "new_style_string_format": True,
    }
    alert = CommandAlerter(rule)
    with mock.patch(
        ...
        alert.alert([match])
    assert mock_popen.called_with(
        "/bin/test --arg 1", stdin=subprocess.PIPE, shell=False
    )
    ...
    with mock.patch(
        ...
        alert.alert([match])
    assert mock_popen.called_with(
        "/bin/test/foo.sh {bar}", stdin=subprocess.PIPE, shell=True
    )

    # Test command with pipe_match_json
    rule = {
        "command": ["/bin/test/", "--arg", "%(somefield)s"],
        "pipe_match_json": True,
    }
    alert = CommandAlerter(rule)
    match = {"@timestamp": "2014-01-01T00:00:00", "somefield": "foobarbaz"}
    with mock.patch(
        "elastalert.alerter.command_alerter.subprocess.Popen"
    ) as mock_popen:
...
def test_slack_attach_kibana_discover_url_when_not_generated():
    ...
    rules_loader.load_modules(rule)
    alert = SlackAlerter(rule)
    match = {"@timestamp": "2016-01-01T00:00:00"}
    with mock.patch("requests.post") as mock_post_request:
        alert.alert([match])
...
def test_http_alerter_with_payload():
    ...
    with mock.patch("requests.post") as mock_post_request:
        alert.alert([match])
    expected_data = {"posted_name": "foobarbaz", "name": "somestaticname"}
    mock_post_request.assert_called_once_with(
        rule["http_post_url"],
        data=mock.ANY,
        headers={
...
def test_alert_text_kw_global_substitution(ea):
    ...
    rule["alert_text_kw"] = {"owner": "owner", "foo_rule": "foo", "abc": "abc"}
...
def test_resolving_rule_references(ea):
    rule = {
        ...
        "owner": "the_owner",
        "priority": 2,
        "list_of_things": ["1", "$owner$", ["11", "$owner$"]],
        "nested_dict": {"nested_one": "1", "nested_owner": "$owner$"},
        "resolved_string_reference": "$owner$",
        "resolved_int_reference": "$priority$",
        "unresolved_reference": "$foo$",
    }
    alert = TestAlerter(rule)
...
def test_hipchat_body_size_limit_text():
    rule = {
        ...
        "alert_text_type": "alert_text_only",
        "alert": [],
        "alert_text_kw": {"@timestamp": "time", "message": "message"},
    }
    rules_loader = FileRulesLoader({})
    rules_loader.load_modules(rule)
    alert = HipChatAlerter(rule)
    match = {"@timestamp": "2018-01-01T00:00:00", "message": "foo bar\n" * 5000}
...
def test_alert_subject_size_limit_no_args(ea):
    ...
    alert = TestAlerter(rule)
    alertSubject = alert.create_custom_title(
        [{"test_term": "test_value", "@timestamp": "2014-10-31T00:00:00"}]
    )
