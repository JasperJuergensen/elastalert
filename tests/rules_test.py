...
def test_new_term():
    ...
    for call in call_args:
        ...
        if old_field != field:
            old_field = field
            old_ts = "2010-01-01T00:00:00Z"
        gte = call[1]["body"]["aggs"]["filtered"]["filter"]["bool"]["must"][0]["range"][
            "@timestamp"
        ]["gte"]
        assert gte > old_ts
        lt = call[1]["body"]["aggs"]["filtered"]["filter"]["bool"]["must"][0]["range"][
            "@timestamp"
        ]["lt"]
        assert lt > gte
        old_ts = gte
...
def test_percentage_match():
    ...
    rule.check_matches(
        datetime.datetime.now(), None, create_percentage_match_agg(None, 100)
    )
    rule.check_matches(
        datetime.datetime.now(), None, create_percentage_match_agg(26, 74)
    )
    rule.check_matches(
        datetime.datetime.now(), None, create_percentage_match_agg(74, 26)
    )

    assert len(rule.matches) == 0

    rule.check_matches(
        datetime.datetime.now(), None, create_percentage_match_agg(24, 76)
    )
    rule.check_matches(
        datetime.datetime.now(), None, create_percentage_match_agg(76, 24)
    )
    assert len(rule.matches) == 2
    ...
    rule = PercentageMatchRule(rules)
    rule.check_matches(
        datetime.datetime.now(), "qk_val", create_percentage_match_agg(76.666666667, 24)
    )
    assert rule.matches[0]["qk"] == "qk_val"
