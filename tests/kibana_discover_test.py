...
@pytest.mark.parametrize(
    "kibana_version",
    ["5.6", "6.0", "6.1", "6.2", "6.3", "6.4", "6.5", "6.6", "6.7", "6.8"],
)
def test_generate_kibana_discover_url_with_kibana_5x_and_6x(kibana_version):
    url = generate_kibana_discover_url(
        ...
        match={"timestamp": "2019-09-01T00:30:00Z"},
    )
    expectedUrl = (
...
def test_generate_kibana_discover_url_with_kibana_discover_app_url_env_substitution(
    environ,
):
    environ.update({"KIBANA_HOST": "kibana", "KIBANA_PORT": "5601"})
    url = generate_kibana_discover_url(
        rule={
...
def test_generate_kibana_discover_url_with_from_timedelta():
    url = generate_kibana_discover_url(
        ...
        match={"timestamp": "2019-09-01T04:00:00Z"},
    )
    expectedUrl = (
        "http://kibana:5601/#/discover"
        + "?_g=%28"  # global start
...
def test_generate_kibana_discover_url_with_timeframe():
    url = generate_kibana_discover_url(
        ...
        match={"timestamp": "2019-09-01T04:30:00Z"},
    )
    expectedUrl = (
        "http://kibana:5601/#/discover"
        + "?_g=%28"  # global start
...
def test_generate_kibana_discover_url_with_multiple_filters():
    url = generate_kibana_discover_url(
        rule={
            ...
            "timestamp_field": "timestamp",
            "filter": [{"term": {"app": "test"}}, {"term": {"level": 30}}],
        },
        match={"timestamp": "2019-09-01T00:30:00Z"},
    )
    expectedUrl = (
...
def test_generate_kibana_discover_url_with_int_query_key():
    url = generate_kibana_discover_url(
        ...
        match={"timestamp": "2019-09-01T00:30:00Z", "geo.dest": 200},
    )
    expectedUrl = (
        "http://kibana:5601/#/discover"
        + "?_g=%28"  # global start
...
def test_generate_kibana_discover_url_with_str_query_key():
    url = generate_kibana_discover_url(
        ...
        match={"timestamp": "2019-09-01T00:30:00Z", "geo": {"dest": "ok"}},
    )
    expectedUrl = (
        "http://kibana:5601/#/discover"
        + "?_g=%28"  # global start
...
def test_generate_kibana_discover_url_with_null_query_key_value():
    url = generate_kibana_discover_url(
        ...
        match={"timestamp": "2019-09-01T00:30:00Z", "status": None},
    )
    expectedUrl = (
        "http://kibana:5601/#/discover"
        + "?_g=%28"  # global start
...
def test_generate_kibana_discover_url_with_compound_query_key():
    url = generate_kibana_discover_url(
        ...
        match={"timestamp": "2019-09-01T00:30:00Z", "geo": {"src": "CA", "dest": "US"}},
    )
    expectedUrl = (
        "http://kibana:5601/#/discover"
        + "?_g=%28"  # global start
...
def test_generate_kibana_discover_url_with_filter_and_query_key():
    url = generate_kibana_discover_url(
        ...
        match={"timestamp": "2019-09-01T00:30:00Z", "status": "ok"},
    )
    expectedUrl = (
        "http://kibana:5601/#/discover"
        + "?_g=%28"  # global start
