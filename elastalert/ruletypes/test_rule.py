...
class MockElastAlerter(object):
    ...
    def test_file(self, conf, args):
        ...
        try:
            res = es_client.count(
                index, doc_type=doc_type, body=count_query, ignore_unavailable=True
            )
        except Exception as e:
        ...
        if include:
            for term in include:
                if not lookup_es_key(terms, term) and "*" not in term:
                    print(
                        "Included term %s may be missing or null" % (term),
                        file=sys.stderr,
                    )
        ...
        if (args.save or args.formatted_output) and not args.count:
            try:
                res = es_client.search(
                    index, size=args.max_query_size, body=query, ignore_unavailable=True
                )
            except Exception as e:
                print("Error running your filter:", file=sys.stderr)
                print(repr(e)[:2048], file=sys.stderr)
    ...
    def run_elastalert(self, rule, conf, args):
        ...
        if args.json:
            ...
            try:
                ...
                print(
                    "All documents must have a timestamp and _id: %s" % (e),
                    file=sys.stderr,
                )
                if args.stop_error:
                    exit(4)
        ...
        if (
            not rule.get("use_terms_query")
            and not rule.get("use_count_query")
        ...
        with mock.patch.object(client, "writeback") as mock_writeback:
            ...
            if mock_writeback.call_count:
                ...
                for call in mock_writeback.call_args_list:
                    if args.formatted_output:
                        self.formatted_output["writeback"][call[0][0]] = json.loads(
    ...
    def run_rule_test(self):
        ...
        parser.add_argument(
            "--config", action="store", dest="config", help="Global config file."
        )
