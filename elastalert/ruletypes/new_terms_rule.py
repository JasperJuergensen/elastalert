...
from elastalert.utils.time import ts_to_dt, ts_now
from elastalert.utils.util import elasticsearch_client, add_raw_postfix, format_index, lookup_es_key
        ...
        if self.rules.get('use_terms_query'):
            ...
            if not self.rules.get('query_key').endswith('.keyword') and not self.rules.get('query_key').endswith('.raw'):
                if self.rules.get('use_keyword_postfix', True):
                    log.warn('Warning: If query_key is a non-keyword field, you must set '
