...
kibana5_kibana6_versions = frozenset(
...
def generate_kibana_discover_url(rule, match):
    ...
    if kibana_version in kibana5_kibana6_versions:
        ...
        logging.warning(
            ...
            % (kibana_version, rule.get("name", "<MISSING NAME>"))
        )
        return None
    ...
    return prison.dumps(
        {
            ...
            "time": {"from": from_time, "mode": "absolute", "to": to_time},
        }
    )
    ...
    return prison.dumps(
        {
            "filters": [],
            "refreshInterval": {"pause": True, "value": 0},
            "time": {"from": from_time, "to": to_time},
        }
    )
            ...
            app_filters.append(
                {
                    ...
                    "meta": {
                        ...
                        "key": query_key,
                        "negate": False,
                        "params": {"query": query_value, "type": "phrase"},
                        "type": "phrase",
                        "value": str(query_value),
                    },
                    "query": {
                        "match": {query_key: {"query": query_value, "type": "phrase"}}
                    },
                }
            )

    return prison.dumps(
        {"columns": columns, "filters": app_filters, "index": index, "interval": "auto"}
    )
