...
def create_index_mappings(es_client, ea_index, recreate=False, old_ea_index=None):
    ...
    if is_atleastseven(esversion):
        ...
        es_client.indices.put_mapping(
            index=ea_index, doc_type="_doc", body=es_index_mappings["elastalert"]
        )
        es_client.indices.put_mapping(
            index=ea_index + "_status",
            doc_type="_doc",
        ...
        es_client.indices.put_mapping(
            index=ea_index, doc_type="elastalert", body=es_index_mappings["elastalert"]
        )
        es_client.indices.put_mapping(
            index=ea_index + "_status",
        ...
        es_client.indices.put_mapping(
            index=ea_index, doc_type="silence", body=es_index_mappings["silence"]
        )
        es_client.indices.put_mapping(
            index=ea_index,
            doc_type="elastalert_error",
    ...
    if old_ea_index:
        print(
            "Copying all data from old index '{0}' to new index '{1}'".format(
                old_ea_index, ea_index
            )
        )
...
def main():
    ...
    parser.add_argument(
        "--host", default=os.environ.get("ES_HOST", None), help="Elasticsearch host"
    )
    parser.add_argument(
        "--port",
        default=os.environ.get("ES_PORT", None),
        type=int,
        help="Elasticsearch port",
    )
    parser.add_argument(
        "--username",
        default=os.environ.get("ES_USERNAME", None),
        help="Elasticsearch username",
    )
    parser.add_argument(
        "--password",
        default=os.environ.get("ES_PASSWORD", None),
    ...
    parser.add_argument(
        "--ssl", action="store_true", default=env("ES_USE_SSL", None), help="Use TLS"
    )
    parser.add_argument(
        "--no-ssl", dest="ssl", action="store_false", help="Do not use TLS"
    )
    parser.add_argument(
        "--verify-certs",
        action="store_true",
    ...
    parser.add_argument(
        "--timeout", default=60, type=int, help="Elasticsearch request timeout"
    )
    parser.add_argument(
        "--config",
        default="config.yaml",
    ...
    if filename:
        ...
        if use_ssl:
            verify_certs = (
                args.verify_certs
                if args.verify_certs is not None
                else input("Verify TLS certificates? t/f: ").lower()
                not in ("f", "false")
            )
        else:
            verify_certs = True
        ...
        index = (
            ...
            else input("New index name? (Default elastalert_status) ")
        )
        if not index:
        ...
        alias = (
            ...
            else input("New alias name? (Default elastalert_alerts) ")
        )
        if not alias:
    ...
    create_index_mappings(
        es_client=es, ea_index=index, recreate=args.recreate, old_ea_index=old_index
    )
