...
class JiraAlerter(Alerter):
    ...
    def set_priority(self):
        try:
            ...
            log.error("Priority %s not found. Valid priorities are %s" % (self.priority, list(self.priority_ids.keys())))
    ...
    def find_existing_ticket(self, matches):
        ...
        try:
            ...
            log.exception("Error while searching for JIRA ticket using jql '%s': %s" % (jql, e))
            return None
            ...
            if ticket:
                ...
                for match in matches:
                    try:
                        ...
                        log.exception("Error while commenting on ticket %s: %s" % (ticket, e))
                    if self.labels:
                        for l in self.labels:
                            try:
                                ticket.fields.labels.append(l)
                            except JIRAError as e:
                                log.exception("Error while appending labels to ticket %s: %s" % (ticket, e))
                if self.transition:
                    log.info('Transitioning existing ticket %s' % (ticket.key))
                    try:
                        self.transition_ticket(ticket)
                    except JIRAError as e:
                        log.exception("Error while transitioning ticket %s: %s" % (ticket, e))
