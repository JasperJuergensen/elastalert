...
from elastalert.utils.util import EAException, lookup_es_key
...
class OpsGenieAlerter(Alerter):
    ...
    def __init__(self, *args):
        ...
        self.tags = self.rule.get("opsgenie_tags", []) + [
            "ElastAlert",
            self.rule["name"],
        ]
        self.to_addr = self.rule.get(
    ...
    def _parse_responders(
        self, responders, responder_args, matches, default_responders
    ):
        if responder_args:
            formated_responders = list()
            responders_values = dict(
            ...
            for responder in responders:
                ...
                try:
                    ...
                    log.warn(
                        "OpsGenieAlerter: Cannot create responder for OpsGenie Alert. Key not foud: %s. "
                        % (error)
                    )
            if not formated_responders:
                log.warn(
                    "OpsGenieAlerter: no responders can be formed. Trying the default responder "
                )
    ...
    def alert(self, matches):
        ...
        self.recipients = self._parse_responders(
            self.recipients, self.recipients_args, matches, self.default_reciepients
        )
        self.teams = self._parse_responders(
            self.teams, self.teams_args, matches, self.default_teams
        )
        post = {}
        ...
        if self.recipients:
            post["responders"] = [
                {"username": r, "type": "user"} for r in self.recipients
            ]
        if self.teams:
    ...
    def create_custom_title(self, matches):
        ...
        if self.opsgenie_subject_args:
            opsgenie_subject_values = [
                lookup_es_key(matches[0], arg) for arg in self.opsgenie_subject_args
            ]
