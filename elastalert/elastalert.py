...
class ElastAlerter(object):
    ...
    def get_query(
        ...
        if starttime and endtime:
            es_filters["filter"]["bool"]["must"].insert(
                0, {"range": {timestamp_field: {"gt": starttime, "lte": endtime}}}
            )
        if five:
    ...
    def get_aggregation_query(
        self, query, rule, query_key, terms_size, timestamp_field="@timestamp"
    ):
        """ Takes a query generated by get_query and outputs a aggregation query """
        query_element = query["query"]
        if "sort" in query_element:
    ...
    def get_index_start(self, index, timestamp_field="@timestamp"):
        ...
        try:
            ...
            self.handle_error(
                "Elasticsearch query error: %s" % (e), {"index": index, "query": query}
            )
    ...
    def process_hits(rule, hits):
        ...
        for hit in hits:
            ...
            set_es_key(
                ...
                lookup_es_key(hit["_source"], rule["timestamp_field"]),
            )
    ...
    def get_hits(self, rule, starttime, endtime, index, scroll=False):
        ...
        try:
            if scroll:
                res = self.thread_data.current_es.scroll(
            ...
            self.handle_error(
                "Error running query: %s" % (e), {"rule": rule["name"], "query": query}
            )
            return None
        hits = res["hits"]["hits"]
        ...
        if self.thread_data.total_hits > rule.get(
            "max_query_size", self.max_query_size
        ):
            log.info("%s (scrolling..)" % status_log)
        else:
            log.info(status_log)
    ...
    def get_hits_count(self, rule, starttime, endtime, index):
        ...
        lt = rule.get("use_local_time")
        log.info(
            "Queried rule %s from %s to %s: %s hits"
            % (
                rule["name"],
                pretty_ts(starttime, lt),
                pretty_ts(endtime, lt),
    ...
    def get_hits_terms(self, rule, starttime, endtime, index, key, qk=None, size=None):
        ...
        if qk:
            ...
            if len(qk_list) == 1:
                ...
                if rule.get("raw_count_keys", True) and not rule["query_key"].endswith(
                    end
                ):
                ...
                for i in range(len(filter_keys)):
                    ...
                    if rule.get("raw_count_keys", True) and not key.endswith(end):
                        key_with_postfix = add_raw_postfix(
                            key_with_postfix, rule["five"]
                        )
    ...
    def get_hits_aggregation(
        self, rule, starttime, endtime, index, query_key, term_size=None
    ):
        rule_filter = copy.copy(rule["filter"])
        base_query = self.get_query(
            rule_filter,
    ...
    def get_starttime(self, rule):
        ...
        try:
            ...
            self.handle_error(
                "Error querying for last run: %s" % (e), {"rule": rule["name"]}
            )
    ...
    def get_segment_size(self, rule):
        ...
        if (
            not rule.get("use_count_query")
            and not rule.get("use_terms_query")
            and not rule.get("aggregation_query_element")
        ):
    ...
    def run_rule(self, rule, endtime, starttime=None):
        ...
        while rule["type"].matches:
            ...
            if self.is_silenced(rule["name"] + "._silence") or self.is_silenced(
                silence_cache_key
            ):
    ...
    def init_rule(self, new_rule, new=True):
        ...
        try:
            ...
            log.warning(
                "Error connecting to Elasticsearch for rule {}. "
        ...
        if "download_dashboard" in new_rule["filter"]:
            ...
            if db_filters is not None:
                ...
                raise EAException(
                    "Could not download filters from %s"
    ...
    def load_rule_changes(self):
        ...
        for rule_file, hash_value in self.rule_hashes.items():
            ...
            if hash_value != new_rule_hashes[rule_file]:
                ...
                try:
                    new_rule = self.rules_loader.load_configuration(
                        rule_file, self.conf
                    )
                    if not new_rule:
                    ...
                    continue
                log.info("Reloading configuration for rule %s" % (rule_file))
        ...
        if not self.args.rule:
            for rule_file in set(new_rule_hashes.keys()) - set(self.rule_hashes.keys()):
                ...
                if self.init_rule(new_rule):
                    log.info("Loaded new rule %s" % (rule_file))
    ...
    def handle_config_change(self):
        if not self.args.pin_rules:
            ...
            log.info(
                "Background configuration change check run at %s"
                % (pretty_ts(ts_now()))
            )
    ...
    def handle_rule_execution(self, rule):
        ...
        if rule.get("limit_execution"):
            ...
            if endtime_epoch + rule["run_every"].total_seconds() < exec_next - 59:
                ...
                rule["next_starttime"] = datetime.datetime.utcfromtimestamp(
                    exec_next
        ...
        try:
            ...
            self.handle_error(
                "Error running rule %s: %s" % (rule["name"], e), {"rule": rule["name"]}
            )
    ...
    def upload_dashboard(self, db, rule, match):
        ...
        if not kibana_url:
            kibana_url = "http://%s:%s/_plugin/kibana/" % (
                rule["es_host"],
                rule["es_port"],
            )
    ...
    def get_dashboard(self, rule, db_name):
        ...
        try:
            ...
            raise EAException("Error querying for dashboard: %s" % (e)).with_traceback(
                sys.exc_info()[2]
            )
    ...
    def alert(self, matches, rule, alert_time=None, retried=False):
        ...
        try:
            return self.send_alert(
                matches, rule, alert_time=alert_time, retried=retried
            )
        except Exception as e:
            self.handle_uncaught_exception(e, rule)
            ...
            for match in matches:
                ...
                if isinstance(rule["type"], FlatlineRule):
                    ...
                    timeframe = datetime.timedelta(
                        seconds=2 * rule.get("timeframe").total_seconds()
                    )
                else:
                    timeframe = rule.get("timeframe", datetime.timedelta(minutes=10))

                start = (
                    ts_to_dt(lookup_es_key(match, rule["timestamp_field"])) - timeframe
                )
                end = ts_to_dt(
                    lookup_es_key(match, rule["timestamp_field"])
                ) + datetime.timedelta(minutes=10)
                ...
                for aggregation_key_value, aggregate_alert_time in rule[
                    ...
                    if ts_now() > aggregate_alert_time:
                        alertable_matches = [
                            ...
                            if self.get_aggregation_key_value(rule, agg_match)
        ...
        if res["hits"]["hits"]:
            ...
            if rule_name not in list(self.silence_cache.keys()):
                ...
                self.silence_cache[rule_name] = (
                    ts_to_dt(until_ts),
                    self.silence_cache[rule_name][1],
                )
    ...
    def handle_uncaught_exception(self, exception, rule):
        ...
        if self.disable_rules_on_error:
            self.rules = [
                running_rule
                for running_rule in self.rules
                if running_rule["name"] != rule["name"]
            ]
